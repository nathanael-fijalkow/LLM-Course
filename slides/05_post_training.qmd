...
The full KL is over all possible responses, so it is not computed exactly.

Instead, we estimate it on the sampled response $y = (y_1, \ldots, y_T)$:

$$
\log \frac{\pi_\theta(y \mid x)}{\pi_\text{ref}(y \mid x)}
= \sum_{t=1}^{T}
\log \frac{\pi_\theta(y_t \mid y_{<t}, x)}{\pi_\text{ref}(y_t \mid y_{<t}, x)}
$$

. . .

This is just the sum of **per-token log-probability differences**. It is cheap to compute while keeping the policy close to the reference model and preventing reward hacking.
...
For a 70B model, this is ~4 × 140 GB ≈ 560 GB of GPU memory. 
...
KTO uses a single score relative to a reference baseline:

$$s(x,y) = \log \frac{\pi_\theta(y\mid x)}{\pi_\text{ref}(y\mid x)} - z_\text{ref}$$

where $z_\text{ref}$ is the batch-average log-ratio.

Then the loss is:

$$
\mathcal{L}_{\text{KTO}}
= \lambda_D \, \mathbb{E}_{\text{good}}\left[1 - \sigma(\beta s(x,y))\right]
+ \lambda_U \, \mathbb{E}_{\text{bad}}\left[1 - \sigma(-\beta s(x,y))\right]
$$

. . .

Good outputs are rewarded when they are above the reference baseline; bad outputs are penalized when they are above that baseline.

## KTO intuition
<!-- type: content -->

- No pairwise comparison is required.
- Each example is labeled only as **good** or **bad**.
- The model is trained to move good completions up and bad completions down relative to the reference model.
- This is especially useful when preferences are noisy or hard to pair.
...
## LoRA: target matrices and delta weights
<!-- type: content -->

For a matrix we want to adapt, such as a transformer weight $W \in \mathbb{R}^{d \times d}$, we keep the base model frozen and add a correction term:

$$W' = W + \Delta W$$

The original weights stay fixed. We only learn the small update $\Delta W$.

. . .

This is the key idea of LoRA: **adapt a subset of matrices**, not the whole model.

## LoRA (Low-Rank Adaptation)
<!-- type: content -->

Instead of learning a dense update $\Delta W \in \mathbb{R}^{d \times d}$ directly, we factorize it as a low-rank product:
