...
## The 2026 ecosystem: choose the layer
<!-- type: content -->
...
The framework runs the model/tool loop. The application still owns permissions and side effects.
...
All of these packages still require authorization, validation, evaluation, and an explicit side-effect policy.
...
| Concept | Purpose |
|---|---|
| Tool schemas | Describe and validate possible actions |
| Agent loop | Iterate over perception, action, and observation |
| Context engineering | Manage state within a finite context budget |
| Sandboxing and policy | Limit the consequences of model mistakes |
| Evaluation and tracing | Measure system reliability and diagnose failures |
| Orchestration | Combine deterministic control and specialized delegation |
| MCP | Standardize integration of context and capabilities |

**Next**: Grammar Alignment, constraining LLM outputs to follow formal grammars.