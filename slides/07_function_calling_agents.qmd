...
- **Model**: decides the next step from observations and instructions.
- **Tools**: define the valid actions and their arguments.
- **Runtime**: validates tool calls, executes them, and stores state.
- **Observation**: the tool result is fed back into the next model call.
- **Policy**: approvals, retries, limits, and safety checks stay outside the model.

## What not to do
<!-- type: content -->

- Trust model-generated arguments without validation.
- Treat chat history as durable memory.
- Repeat mutating actions without idempotency or approval.
- Let tool output redefine system instructions or permissions.
- Benchmark only the model, not the full agent system.

Goal: design a reliable control loop.
...
A production-ready agent can be surprisingly small:

1. Keep a message history and task state.
2. Call the model with a tool list and policy.
3. Validate each tool call before execution.
4. Execute the tool in a controlled environment.
5. Feed the result back and continue until the task completes or a stop condition is reached.
...
while not done and steps < max_steps:
    ...
    for call in response.tool_calls:
        ...
        messages.extend(as_tool_messages(result))
    done = response.text is not None and no_tool_calls
    steps += 1
```

This is the minimal control loop. Everything else is policy, memory, or orchestration.
...
The loop runtime must distinguish at least four kinds of state:
...
- **Artifacts**: files, code, plans, and checkpoints outside the prompt.

Do not treat the chat history as memory. The runtime owns state explicitly.
