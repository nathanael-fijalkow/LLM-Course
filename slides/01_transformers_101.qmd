...
If $q$ and $k$ are random vectors, the dot product grows roughly like $d_k$.

So the raw attention scores can become very large and the softmax saturates:

$$\alpha_{i,j} = \frac{q_i \cdot k_j}{\sqrt{d_k}}$$

Dividing by $\sqrt{d_k}$ keeps the score scale roughly constant, which makes softmax smoother and training more stable.
...
So a length-$T$ sequence gives $T$ next-token predictions, each conditioned on a different sliding window of past tokens.

. . .

The key point is simple: **the model reads a context window, predicts the next token, then shifts one step forward.**
...
These are three ways to reshape the next-token distribution before sampling:

- **Temperature**: sharpens or flattens logits.
...
```python
logits = model(...).logits[:, -1, :] / temperature
...
These knobs let us trade off **creativity** against **coherence**.
